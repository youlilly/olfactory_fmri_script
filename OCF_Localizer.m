...
% Clear back page
cgrect(0, 0, ScrWid, ScrHgh, [1 1 1]) %Creates a screen-sized white rectangle, effectively "clearing" the screen.
...
cgpencol(0,0,0);
cgtext('Press the GREEN button if you think',0,ScrHgh / 6 - 15);%should be the blue button
cgtext('there IS a smell',0,-15);
cgtext('Press the YELLOW button if you think',0,-1*ScrHgh/6 - 15);%this should be the yellow/fuzzy button
...
%allows them to prepare for the sniff.
    ...
    odordurTimes_indodorind{odorid} = [odordurTimes_indodorind{odorid} odor_off-odor_on]; %Collects the odor duration time and sorts it into the specific odor condition
    ...
    if button_pressed    % If any button response was recorded, byte_in should be greater than 0.
        ...
        if (odorid < 5 && response == 1) % i.e. correct to press 'yes' right button
            ...
            yes_odordurTimes{odorid}  = [yes_odordurTimes{odorid} odor_off-odor_on]; %Records odor duration time for the correct response
            ...
            yes_odordurTimes_present{odorpres} = [yes_odordurTimes_present{odorpres} odor_off-odor_on]; %Records odor duration time for the correct response of any odor
        ...
        elseif (odorid == 5 && response == 0) % i.e. correct to press 'no' left button
            ...
            no_odordurTimes = [no_odordurTimes odor_off-odor_on]; %Records odor duration time for the correct rejection
        ...
        elseif (odorid < 5 && response == 0) % i.e. incorrect to press 'no' left button
            ...
            miss_durTimes = [miss_durTimes odor_off-odor_on]; %Records the odor duration times for the missed odors
        ...
        elseif (odorid == 5 && response == 1) % i.e. incorrect to press 'yes' right button
            ...
            fa_durTimes = [fa_durTimes odor_off-odor_on]; %Records the duration times for the false alarms
        ...
        noresp_durTimes = [noresp_durTimes odor_off-odor_on]; %Records the trial duration time for no response trials
