...
for i=1:length(stimord)
    ...
    if odorpres == 1 %Odor present = yes
        odorval = stimord(i,3);
        if (odorid > 0) && (odorid < 4) %for odor IDs 1,2,3 (Neutral Sweat)%are three ids point to the same odor?
    ...
    greeid = stimord(i,7);
    if greeid == 0 %greeid will never be 0?
        greeid = 99;
    end
    greeval = stimord(i,8); 
    greebleindex = [greebleindex, greeid];
    greeblevalindex = [greeblevalindex, greeval];
    
    if greeval == 0 %greeval will never be 0?
        greecond = 5;
    else
        greecond = greeval;
    end
    greeblecondindex = [greeblecondindex greecond];
    ...
    if odorpres == 1 && picpres == 0
        ...
        % odor onset
        odoronTimes_greeval{greecond} = [odoronTimes_greeval{greecond} tO];
        odordurTimes_greeval{greecond} = [odordurTimes_greeval{greecond} tOoff-odor_on];
        odoronMRtimes_greeval{greecond} = [odoronMRtimes_greeval{greecond} tO-starttime];
    ...
    elseif odorpres == 0 && picpres == 1
        ...
        piconTimes_greeval{greecond} = [piconTimes_greeval{greecond} tPon];
        picdurTimes_greeval{greecond} = [picdurTimes_greeval{greecond} tPoff-tPon];
        piconMRtimes_greeval{greecond} = [piconMRtimes_greeval{greecond} tPon-starttime];
    ...
    % greeble onset
    greebleonTimes_greeval{greecond} = [greebleonTimes_greeval{greecond} gree_on];
    greebledurTimes_greeval{greecond} = [greebledurTimes_greeval{greecond} tgoff-gree_on];
    greebleonMRtimes_greeval{greecond} = [greebleonMRtimes_greeval{greecond} gree_on-starttime];
    ...
    if button_pressed == true    % If any button response was recorded, byte_in should be greater than 0.
        ...
        if odorpres == 1 && picpres == 0 % Odor condition
            if response == 1
                RTs_greeval{greecond} = [RTs_greeval{greecond} rt];
                RTs_odorval{odorval} = [RTs_odorval{odorval} rt];
                % RTsptb_greeval{greecond} = [RTs_greeval{greecond} rtPTB];
                %RTsptb_odorval{odorval} = [RTs_odorval{odorval} rtPTB];
                respval = 1;
                responsevaluesall = [responsevaluesall respval];
                GreeResp{greecond} = [GreeResp{greecond} respval];
            ...
            elseif response == 4
                ...
                RTs_greeval{greecond} = [RTs_greeval{greecond} NaN];
                RTs_odorval{odorval} = [RTs_odorval{odorval} NaN];
                %RTsptb_greeval{greecond} = [RTs_greeval{greecond} NaN];
